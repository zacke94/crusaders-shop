...
def get_products():
    try:
        products = [{'id': product[0], 'name': product[1], 'price': product[2]} for product in get_products_from_db()]
...
def edit_product():
    try:
        edit_product_db(request.json['id'], request.json['name'], request.json['price'])
        return "Success", 200
    except Exception as e:
